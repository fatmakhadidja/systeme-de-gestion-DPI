...
class BilanBioMesureCreationView(APIView):
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "BilanBiologique not found."},
        ...
        try:
            ...
            return Response(
                {"error": "Laborantin not found."}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        for mesure in mesure_table:
            ...
            static_element = next(
                (item for item in static_table if item["id"] == id_mesure), None
            )
            if not static_element:
                return Response(
                    {"error": f"No static element found for id-mesure {id_mesure}."},
        ...
        return Response(
            {
                "message": "ParametreBioMesure instances created and linked successfully.",
                "created_mesures": ParametreBioMesureSerializer(
                    created_mesures, many=True
                ).data,
            },
...
class Generergraph(APIView):
    def get(self, request, dpi_id, laborantin_id):
        try:
            ...
            laborantin_exists = Laborantin.objects.filter(
                id_laborantin=laborantin_id
            ).exists()
            ...
            if not dpi:
                return Response(
                    {"error": "DPI not found"}, status=status.HTTP_404_NOT_FOUND
                )
            ...
            for consultation in consultations:
                ...
                if bilan_biologique:  # Ensure bilan_biologique exists
                    ...
                    parametres = ParametreBioMesure.objects.filter(
                        bilan_biologique=bilan_id
                    )
            ...
            if counter == 0:
                return Response(
                    {"error": "No parametres found for the given DPI"},
                    status=status.HTTP_404_NOT_FOUND,
                )
            ...
            if counter == 1:
                ...
                parametres_data = [
                    {"nom": param.nom, "valeur_mesuree": param.valeur_mesuree}
                    for param in parametres
                ]
                parametre_serializer = ParametreBioMesureSerializer(
                    parametres, many=True
                )
                return Response({"type": "unaire", "parametres": parametres_data})
            if counter > 1:
                ...
                parametres_bilan1 = ParametreBioMesure.objects.filter(
                    bilan_biologique=bilanid1
                )
                parametres_bilan2 = ParametreBioMesure.objects.filter(
                    bilan_biologique=bilanid2
                )
                ...
                common_params = []
                for nom in (
                    params_bilan1.keys() & params_bilan2.keys()
                ):  # Intersection of keys (common noms)
            ...
            response_data = [
                {
                    ...
                    .values_list("id_consultation", flat=True)
                    .first(),  # Assuming one consultation per bilan
                }
    ...
    def get(self, request, consultation_id):
        try:
            ...
            response_data = {
                ...
                "bilan_biologique": (
                    BilanBiologiqueSerializer(bilan_biologique).data
                    if bilan_biologique
                    else None
                ),
            }
